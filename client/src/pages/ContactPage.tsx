import React, { useState } from 'react';
import { Mail, Phone, MapPin, Send, HelpCircle, ShieldCheck, CheckCircle2 } from 'lucide-react';
import { useToast } from '../context/ToastContext';

export const ContactPage: React.FC = () => {
  const { showToast } = useToast();
  const [name, setName] = useState('');
  const [email, setEmail] = useState('');
  const [subject, setSubject] = useState('technical');
  const [message, setMessage] = useState('');
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [submitted, setSubmitted] = useState(false);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name || !email || !message) {
      showToast('Please fill out all required fields.', 'error');
      return;
    }
    setIsSubmitting(true);
    await new Promise((r) => setTimeout(r, 600));
    setIsSubmitting(false);
    setSubmitted(true);
    showToast('Your message was sent straight to the workbench team!');
  };

  const faqs = [
    {
      q: 'What kind of power supply should I use?',
      a: 'The Neon Fuzz Box runs on standard 9V DC center-negative power (2.1mm barrel, Boss-standard). It also takes an internal 9V alkaline battery. Never feed it more than 9.6V.',
    },
    {
      q: 'Where in my pedalboard signal chain should I place it?',
      a: 'Like classic vintage fuzzes, it loves being FIRST in your signal chain right after your guitar pickups before any buffered pedals or tuners. It also sounds monster pushing an overdrive.',
    },
    {
      q: 'How does the 1-Year Bench Warranty work?',
      a: 'If any potentiometer, footswitch, jack, or internal solder trace fails during normal use within 12 months, we fix or replace it free. You simply ship it to our Kolkata workshop.',
    },
    {
      q: 'How long does shipping take?',
      a: 'All orders placed before 2 PM IST are dispatched same day or next morning via Blue Dart Express / Shiprocket with live tracking. Typical transit time is 2-4 business days.',
    },
  ];

  return (
    <div className="bg-[#0B0B0A] text-[#F3EFE6] pt-24 pb-20">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 space-y-16">
        {/* Header */}
        <div className="text-center space-y-4 pt-6">
          <div className="inline-flex items-center gap-2 px-3 py-1 bg-[#171513] border border-[#8C857A]/30 rounded-full text-xs font-mono-tech uppercase tracking-widest text-[#D91E18]">
            Workshop Direct Support
          </div>
          <h1 className="text-4xl sm:text-6xl font-cinzel font-black tracking-tight uppercase">
            Talk To The Builders
          </h1>
          <p className="text-xs sm:text-base text-[#8C857A] max-w-xl mx-auto">
            Got a question about rig compatibility, custom orders, or your shipment? We answer every message directly from our Kolkata workbench.
          </p>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          {/* Contact Details & Info */}
          <div className="lg:col-span-5 space-y-8">
            <div className="bg-[#171513] border border-[#8C857A]/25 rounded-2xl p-6 sm:p-8 space-y-6">
              <h3 className="text-xl font-bold font-cinzel text-[#F3EFE6]">
                Workbench Headquarters
              </h3>

              <div className="space-y-4 text-xs sm:text-sm">
                <div className="flex items-start gap-3">
                  <div className="p-2 rounded bg-[#0B0B0A] text-[#D91E18] shrink-0 border border-[#8C857A]/20">
                    <MapPin size={16} />
                  </div>
                  <div>
                    <div className="font-bold text-[#F3EFE6]">Kolkata Workshop</div>
                    <div className="text-[#8C857A]">
                      IDIOT Pedals Audio Labs, Southern Avenue Sector, Kolkata, West Bengal 700029, India
                    </div>
                  </div>
                </div>

                <div className="flex items-start gap-3">
                  <div className="p-2 rounded bg-[#0B0B0A] text-[#D91E18] shrink-0 border border-[#8C857A]/20">
                    <Mail size={16} />
                  </div>
                  <div>
                    <div className="font-bold text-[#F3EFE6]">Email Dispatch</div>
                    <div className="text-[#8C857A]">workbench@idiotpedals.com</div>
                    <div className="text-[11px] text-[#8C857A]/70">Typical response within 2 hours</div>
                  </div>
                </div>

                <div className="flex items-start gap-3">
                  <div className="p-2 rounded bg-[#0B0B0A] text-[#D91E18] shrink-0 border border-[#8C857A]/20">
                    <Phone size={16} />
                  </div>
                  <div>
                    <div className="font-bold text-[#F3EFE6]">Phone & WhatsApp</div>
                    <div className="text-[#8C857A]">+91 98301 23456</div>
                    <div className="text-[11px] text-[#8C857A]/70">Mon – Sat, 10:00 AM to 7:00 PM IST</div>
                  </div>
                </div>
              </div>

              <div className="p-4 bg-[#0B0B0A] border border-[#8C857A]/20 rounded-xl flex items-center gap-3 text-xs text-[#8C857A]">
                <ShieldCheck size={18} className="text-[#D91E18] shrink-0" />
                <span>All pedals covered by 1-Year Comprehensive Bench Warranty.</span>
              </div>
            </div>
          </div>

          {/* Contact Form */}
          <div className="lg:col-span-7 bg-[#171513] border border-[#8C857A]/25 rounded-2xl p-6 sm:p-10 shadow-2xl">
            {submitted ? (
              <div className="text-center py-12 space-y-4">
                <CheckCircle2 size={48} className="text-emerald-400 mx-auto" />
                <h3 className="text-2xl font-bold font-cinzel text-[#F3EFE6]">
                  Message Logged On Our Bench
                </h3>
                <p className="text-xs sm:text-sm text-[#8C857A] max-w-md mx-auto">
                  Thank you, {name}. A member of our engineering team has received your ticket and will write back to {email} shortly.
                </p>
                <button
                  onClick={() => setSubmitted(false)}
                  className="px-6 py-2.5 bg-[#0B0B0A] border border-[#8C857A]/30 text-xs font-mono-tech text-[#F3EFE6] rounded uppercase hover:border-[#D91E18]"
                >
                  Send Another Inquiry
                </button>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-5">
                <div className="space-y-1">
                  <h3 className="text-xl font-bold font-cinzel text-[#F3EFE6]">
                    Direct Ticket Submission
                  </h3>
                  <p className="text-xs text-[#8C857A]">
                    Leave your notes, rig setup questions, or order feedback below.
                  </p>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <label className="text-xs font-mono-tech uppercase text-[#8C857A] block">
                      Your Name *
                    </label>
                    <input
                      type="text"
                      required
                      placeholder="e.g. Rahul Sharma"
                      value={name}
                      onChange={(e) => setName(e.target.value)}
                      className="w-full bg-[#0B0B0A] border border-[#8C857A]/30 rounded-lg px-3.5 py-2.5 text-xs text-[#F3EFE6] focus:outline-none focus:border-[#D91E18]"
                    />
                  </div>

                  <div className="space-y-1.5">
                    <label className="text-xs font-mono-tech uppercase text-[#8C857A] block">
                      Email Address *
                    </label>
                    <input
                      type="email"
                      required
                      placeholder="guitarist@gmail.com"
                      value={email}
                      onChange={(e) => setEmail(e.target.value)}
                      className="w-full bg-[#0B0B0A] border border-[#8C857A]/30 rounded-lg px-3.5 py-2.5 text-xs text-[#F3EFE6] focus:outline-none focus:border-[#D91E18]"
                    />
                  </div>
                </div>

                <div className="space-y-1.5">
                  <label className="text-xs font-mono-tech uppercase text-[#8C857A] block">
                    Inquiry Subject
                  </label>
                  <select
                    value={subject}
                    onChange={(e) => setSubject(e.target.value)}
                    className="w-full bg-[#0B0B0A] border border-[#8C857A]/30 rounded-lg px-3.5 py-2.5 text-xs text-[#F3EFE6] focus:outline-none focus:border-[#D91E18]"
                  >
                    <option value="technical">Circuit Compatibility & Rig Troubleshooting</option>
                    <option value="order">Order Tracking & Shipping Inquiry</option>
                    <option value="warranty">Warranty Claim / Repair Service</option>
                    <option value="bulk">Band / Studio Bulk Inquiries</option>
                  </select>
                </div>

                <div className="space-y-1.5">
                  <label className="text-xs font-mono-tech uppercase text-[#8C857A] block">
                    Message *
                  </label>
                  <textarea
                    required
                    rows={4}
                    placeholder="Tell us what guitar/amp you play, or any questions about the Neon Fuzz Box..."
                    value={message}
                    onChange={(e) => setMessage(e.target.value)}
                    className="w-full bg-[#0B0B0A] border border-[#8C857A]/30 rounded-lg p-3 text-xs text-[#F3EFE6] focus:outline-none focus:border-[#D91E18]"
                  />
                </div>

                <button
                  type="submit"
                  disabled={isSubmitting}
                  className="w-full py-3.5 bg-[#D91E18] hover:bg-[#b51712] text-white text-xs font-mono-tech font-bold uppercase rounded flex items-center justify-center gap-2 shadow-lg shadow-[#D91E18]/25 transition-colors"
                >
                  <Send size={14} />
                  {isSubmitting ? 'Transmitting to Workbench...' : 'Send Message To Builders'}
                </button>
              </form>
            )}
          </div>
        </div>

        {/* FAQs */}
        <div id="warranty" className="space-y-8 pt-8">
          <div className="text-center space-y-2">
            <h3 className="text-2xl sm:text-3xl font-cinzel font-bold text-[#F3EFE6]">
              Frequently Asked Questions
            </h3>
            <p className="text-xs text-[#8C857A]">
              Quick answers regarding power, signal chain, and our 1-year warranty.
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {faqs.map((faq, i) => (
              <div key={i} className="p-5 bg-[#171513] border border-[#8C857A]/20 rounded-xl space-y-2">
                <div className="font-bold text-xs sm:text-sm text-[#F3EFE6] flex items-center gap-2">
                  <HelpCircle size={15} className="text-[#D91E18] shrink-0" />
                  {faq.q}
                </div>
                <p className="text-xs text-[#8C857A] leading-relaxed pl-6">{faq.a}</p>
              </div>
            ))}
          </div>
        </div>
      </div>
    </div>
  );
};
